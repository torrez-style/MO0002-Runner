...
def _data_path(nombre_archivo: str) -> str:
...
class MenuPrincipal:
        ...
        if nombre_archivo and os.path.exists(nombre_archivo):
            try:
                ...
                if self._validar_estructura_laberinto(datos):
                    with open(_data_path("niveles.json"), "w", encoding="utf-8") as archivo_salida:
    ...
    def _reiniciar_salon_fama(self):
        try:
            with open(_data_path("puntuaciones.json"), "w", encoding="utf-8") as archivo:
                json.dump([], archivo)
