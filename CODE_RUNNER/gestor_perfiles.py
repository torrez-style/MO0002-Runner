...
class GestorPerfiles:
    ...
    def _buscar_archivo(self, ruta: str) -> str:
        """Busca archivo primero en raíz, luego en CODE_RUNNER."""
        posibles = [ruta, os.path.join("CODE_RUNNER", ruta)]
        return next((p for p in posibles if os.path.exists(p)), ruta)
    ...
    def _guardar_json(self, ruta: str, datos: List[Dict]) -> bool:
        ...
        try:
            # Guardar siempre en CODE_RUNNER para consistencia
            ruta_destino = os.path.join("CODE_RUNNER", os.path.basename(ruta))
